...
if(isset($_POST['simpan'])){
    ...
    //cek di bagian db, email sudah ada atau belum
    if($email != ''){
        $sql1   = "select email from members where email = '$email'";
        $q1     = mysqli_query($koneksi,$sql1);
        $n1     = mysqli_num_rows($q1);
        if($n1 > 0){
            $err .= "<li>Email yang sudah kamu masukan sudah terdaftar</li>";
        }
    ...
    if(empty($err)){
        $status            = md5(rand(0,100));
        $judul_email       = "Halaman Konfirmasi Email";
        $isi_email         = "Akun yang kmu miliki dengan email <b>$email</b> telah siap digunkan. <br/>";
        $isi_email         .= "sebelumnya silakan melakukan aktifasi email di link bawah ini <br/>";
